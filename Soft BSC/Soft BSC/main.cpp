...
int main()
{
    ...
    vector<string> inFile = {"Hmatrix3.txt", "Hmatrix.csv", "Hmatrix2.csv"};
    vector<string> outFile = {"Hmatrix3_out.csv", "Hmatrix_out.csv", "Hmatrix2_out.csv"};
    ...
    for (int f = 0; f < inFile.size(); f++) {
        ...
        // Simulations
        for (double p = 0.001; p < 1; p += 0.001) {
            int corr = 0;
            int err = 0;
            ...
            p_val.push_back(p);
            p_success.push_back((double)corr/100);
        }
