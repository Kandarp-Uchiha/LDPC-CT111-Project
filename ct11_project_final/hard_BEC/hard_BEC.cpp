...
struct VN_node
{
    int r_bit;
    vector<int> edges;
};// VN node contains a vector of index of edges (i.e all CN's it is connected to) and r_bit = 1 if r=1
                                                                                    // r_bit = 0 if r=0
                                                                                    // r_bit = -1 if r=-1(erasure)
    ...
    for (int f = 0; f < inFile.size(); f++) {
        ...
        for (double p = 0; p < 1; p += 0.01) {
            ...
            for (int sim = 0; sim < Nsim; sim++) {
                vector<int> r = BEC_channel(m,p,n_VN);
