...
{
    vector<vector<int>> H;
    Create_Hmatrix("Hmatrix.csv", H);

    int n_CN = (sizeof(H)/sizeof(H[0]));
    int n_VN = (sizeof(H)/sizeof(H[0][0]))/n_CN;

    vector<int> m(n_VN,0);// All 0 codeword

    double p = 0.9; // for making error graph I will vary p also from 0 to 1 in steps
